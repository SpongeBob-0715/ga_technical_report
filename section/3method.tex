\begin{figure*}[htbp]
    \centering
    \includegraphics[width=0.98\linewidth]{image/framework.pdf}
...
\begin{table}[t]
    ...
    \begin{tabularx}{\linewidth}{@{}p{0.28\linewidth}X@{}}
        ...
        \texttt{file\_write} &
        Create new files or write large content blocks. \\
        \texttt{file\_patch} &
        Apply precise patch updates to existing file regions. \\
        \texttt{web\_scan} &
        Extract the current browser page as simplified HTML or text. \\
        \texttt{web\_execute\_js} &
        Execute JavaScript in the browser and return results. \\
        \texttt{ask\_user} &
        Query the user and wait for a response. \\
        ...
        Trigger long-term memory distillation. \\
        \bottomrule
    \end{tabularx}
\end{table}
